...
class DSDMTS(nn.Module):
    def __init__(self, args):
        super(DSDMTS, self).__init__()
    ...
    def forward(self, batch_x, time_x, is_eval):
        ...
        z = self.pi * z_hat + (1 - self.pi) * z_tilde
        mu_z = self.pi * mu_h_hat + (1 - self.pi) * mu_h_tilde
        var_z = self.pi * var_h_hat + (1 - self.pi) * var_h_tilde

        # variant Dynamic Inference
