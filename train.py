...
from backbones import Autoformer, Informer, Transformer, FEDformer, Linear, iTransformer
from model.jointpgm import jointpgm
from model.jointpgm_mixing import jointpgm_mixing
...
# jointpgm or jointpgm_mixing? jointpgm can be used to test w/o ISE (F) and w/o ISE (A)
if args.series_mixing is False:
    model = jointpgm(args).to(device)
else:
    # set w/o ISE (F) and w/o ISE (A) backbones
    MODEL = args.mixing_model
    model_dict = {'Autoformer': Autoformer, 'Transformer': Transformer, 'Informer': Informer, 
                  'FEDformer': FEDformer, 'Linear': Linear, 'iTransformer': iTransformer}
    mixing_model = model_dict[MODEL].Model(args)
    model = jointpgm_mixing(args, mixing_model).to(device)
...
for epoch in range(args.max_epochs):
    ...
    for i, batch in enumerate(train_loader):
        ...
        # treating var as logvar to avoid nan
        ...
        if args.series_mixing is False:
            loss = loss_r + loss_f + args.belta * loss_kl1 + args.belta * loss_kl2
        else:
            loss = loss_f + args.belta * loss_kl1 + args.belta * loss_kl2
